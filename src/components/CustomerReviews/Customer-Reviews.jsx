...
const CustomerReviews = () => {
  return (
    <div className='flex flex-col items-center w-full my-8 mt-16 text-center  px-8'>
      ...
      <h3 className='mt-2 lato text-[12px] md:text-[16px] xl:text-[24px]  '>Buy and Sell Precious Metal with Confidence and Peace of Mind</h3>
    ...
    <div className='flex flex-col items-start ml-4'>
        <div className='flex flex-row items-center'>
            <p className='lato-700 text-[11px] md:text-[14px] lg:text-[16px] 2xl:text-[20px]'>{review.name}</p>
            <img className="w-2 h-2 md:w-4 md:h-4 ml-2" src={VERIFIED.image} alt={VERIFIED.name} />
    
        </div>
        <span className='text-[10px] md:text-[14px]'>Verified Buyer</span>
    </div>
                        </div>
                        <div className='flex flex-col items-start w-auto'>
                            <span className=' text-[10px] md:text-[10px] lg:text-[14px]'>{review.date}</span>
                            <div className='flex flex-row items-center '>
                                        {
                                Array.from({ length: review.rating }, (_, index) => (
            <img className="w-3 h-3 mr-1" src={STAR.image} alt={STAR.name} />
                    ...
                    <p className='my-4 text-[10px] lg:text-[16px] xl:text-[18px] 2xl:text-[20px]'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus lectus mauris, tincidunt nec efficitur sit amet, sollicitudin et risus. Nam nec lobortis nisi. Maecenas porttitor tristique lacinia. Quisq</p>
                </div>
