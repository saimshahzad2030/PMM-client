...
const Checkout = () => {
  ...
  const [metalVerificationSubtotal,setMetalVerificationSubtotal] = React.useState(0)
  ...
  React.useEffect(() => {
    ...
    setGrandTotal(metalVerificationSubtotal + shippingSubtotal
      + productSubtotal);
  }, []);
          ...
          <div className="  flex flex-col items-start  rounded-sm w-full mt-4">
            ...
            >
              ...
              <div className="border border-gray-300 flex flex-row items-start justify-between w-full px-2 sm:px-8  py-4">
                <div className="flex flex-row items-center">
                  <Checkbox
                    checked={orderData[index][0].metalAuthenticaitonService}
                    onChange={() => { 
                      setMetalVerificationSubtotal(orderData[index][0].metalAuthenticaitonService==true?metalVerificationSubtotal-20:metalVerificationSubtotal+20)
                      setOrderData((prevOrderData) =>
                      prevOrderData.map((group, orderIndex) => {
                        if (orderIndex === index) {
                          return group.map((item) =>
                            item.productId == orderData[index][0].productId
                              ? { ...item, metalAuthenticaitonService: !orderData[index][0].metalAuthenticaitonService }
                              : item
                          );
                        }
                        return group;
                ...
                <span className="text-[12px] sm:text-[16px]">{orderData[index].reduce((accumulator, item) => accumulator + item.price, 0)}</span>
              </div>
              <div className="grid grid-cols-3 w-full">
                ...
                <div className="col-span-3 md:col-span-1 w-full flex flex-col items-start border border-gray-300 px-3 py-4 sm:p-6">
                  ...
                  <TextField
                    ...
                    onChange={(e) => handleMessageChange(orderData[index][0].productId, e.target.value)}
