...
const Cart = ({ data }) => {
  ...
  const [transformedArray, setTransformedArray] = React.useState(
    data.map(
      (group) => group.map((g) => ({ unitPrice:g?.product?.price,image:`${g?.product.images[0].image}`,description:`${g?.product?.description}`,sellerName:`${g?.user.firstName} ${g?.user?.lastName}`,buying: 0, totalProductPrice: 0,productId:g?.product?.id,senderId:g?.product?.sellerId })) // Initialize with buying as 0
    )
  );
                ...
                <Link
                  ...
                  onClick={()=>{dispatch(setCarts(
                    transformedArray.map(
                     (group) => group.filter((g) => {return g.buying != 0 }) // Initialize with buying as 0
                   )
                 ))}}
                >
                  Check Out
