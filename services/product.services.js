...
values.productHighlights.forEach((highlight)=>formData.append("productHighlights", highlight))
...
export const fetchProducts = async () => {
    try {
      ...
      console.log(`${config.BASE_URL}product`)
      const data = await response.json(); 
      return data
    } catch (error) {
      console.log(error);
...
export const fetchProduct = async (id) => {
    try {
      const response = await fetch(
        `${config.BASE_URL}single-product?id=${id}`,
        {
          method: "GET",  
          headers: {
            "Content-Type": "application/json",
